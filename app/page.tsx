      ...
      <section className="py-12 px-6 max-w-7xl mx-auto">
        ...
        >
          {categories.map((c) => (
            <div
              key={c.name}
              className="min-w-[140px] flex-shrink-0 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-border text-center"
            >
              <div className={`flex h-16 w-16 mx-auto items-center justify-center rounded-full text-3xl ${c.tint}`}>
                <img src={c.emoji} alt={c.name} className="h-10 w-10 object-contain" />
              </div>
              <h3 className="mt-3 font-semibold text-gray-800 text-sm">{c.name}</h3>
            </div>
          ))}
        </div>
      </section>
